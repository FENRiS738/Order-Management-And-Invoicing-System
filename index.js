...
function addFieldListeners() {
    ...
    document.getElementById('sub_total').addEventListener('input', function (event) {
        globalObject.sub_total = event.target.value;
    });

    document.getElementById('total_tax').addEventListener('input', function (event) {
        globalObject.total_tax = event.target.value;
    });

    document.getElementById('grand_total').addEventListener('input', function (event) {
...
function setOrderData(data) {
    ...
    orders.forEach((item, index) => {
        ...
        element.innerHTML = `
            <input type="text" class="form-control" name="product_name_${index + 1}" id="product_name" value="${item.product_name.trim()}" required/>
            <div class="">
                <span class="">Description</span><br>
                <input type="text" name="description_${index + 1}" id="description" value="${item.description.trim()}" class="form-control" required>
            </div>

            <div class="">
                <span class="">Quantity</span><br>
                <input type="number" name="quantity_${index + 1}" id="quantity" value="${item.quantity}" class="form-control" required>
            </div>
...
async function handleFormSubmit(event) {
    ...
    try {
        let response = await fetch('https://hook.eu1.make.com/xe35aw38h88eddbo2c8hfcy141kmvm8u', {
            method: 'POST',
            body: formData
        })

        if (!response.ok) {
            alert(`There was an error while submittnig the form: ${response.status}.`);
            formReset();
        }
